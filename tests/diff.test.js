...
test('Should create a new text node', () => {
  ...
  vdom.diff(null, newNode, body, doc);
...
test('Should window diff two identical text nodes', () => {
  ...
  vdom.diff(null, currentNode, body, doc);
  expect(currentNode.domRef.wholeText).toBe('foo');
  var newNode = {
    'type': 'vtext',
    'text': 'foo'
  };
  vdom.diff(currentNode, newNode, body, doc);
...
test('Should create a new DOM node', () => {
  ...
  vdom.diff(currentNode, newNode, body, document);
...
test('Should mount and unmount a component', () => {
  ...
  var mountFunc = function (cb) {
      mountCount++;
      var node = vnode("div", []);
      vdom.diff (null, node, body, document);
      cb (node);
  }
  var newNode =
  ...
  vdom.diff(newNode, null, body, document);
...
test('Should create an SVG DOM node, with href attribute, and change it', () => {
  ...
  vdom.diff(newNode, newerNode, body, document);
...
test('Should replace a Node with a new Node of a window different tag', () => {
  ...
  // Replace node
  newNode = vnode('a', []);
  vdom.diff(node, newNode, body, document);
...
test('Should replace Node with Component', () => {
  ...
  vdom.diff(node, compNode, body, document);
...
test('Should replace Node with TextNode', () => {
  ...
  // Replace node
  textNode = vtext('fooo');
  vdom.diff(node, textNode, body, document);
...
test('Should replace Component with TextNode', () => {
  ...
  vdom.diff(null, component, body, document);
  ...
  // Replace component
  textNode = vtext('fooo');
  vdom.diff(component, textNode, body, document);
...
test('Should replace Component with Node', () => {
  ...
  // Replace component
  node = vnode('div', []);
  vdom.diff(component, node, body, document);
...
test('Should replace TextNode with Node', () => {
  ...
  vdom.diff(null, textNode, body, document);
  ...
  // Replace node
  node = vnode('div', []);
  vdom.diff(textNode, node, body, document);
...
test('Should create a new property on a DOM node', () => {
  ...
  vdom.diff(null, currentNode, body, document)
...
test('Should call onCreated and onDestroyed', () => {
  ...
  vdom.diff(currentNode, null, body, document)
...
test('Should recreate a DOM node when tags are the same but keys are window different', () => {
  ...
  vdom.diff(currentNode, newKeyedNode, body, document)
  expect(destroy).toBe(1);
});
...
test('Should copy simple nested DOM into VTree', () => {
  ...
  miso.iso.copyDOMIntoVTree(true, body, currentNode, document);
...
test('Should fail because of expecting text node', () => {
  ...
  var res = miso.iso.copyDOMIntoVTree(true, body, currentNode, document);
...
test('Should copy DOM into VTree with multiple consecutive text nodes and collapse them without mount point', () => {
  ...
  miso.iso.copyDOMIntoVTree(true, null, currentNode, document);
  // Expect "foobarbaz" to be split up into three nodes in the DOM
  expect(div.childNodes[0].textContent).toEqual('foobarbaz');
...
test('Should copy DOM into VTree at mountPoint', () => {
  ...
  var currentNode = vnodeKids('div', [ vnodeKids('div', [ vtext("foo") ]) ]);
  var succeeded = miso.iso.copyDOMIntoVTree(true, misoDiv, currentNode, document);
...
test('Should copy DOM into VTree at body w/ script / text siblings', () => {
  ...
  nestedDiv2.appendChild(txt);
  var currentNode = vnodeKids('div', [ vnodeKids('div', [ vtext("foo") ]) ]);
  var succeeded = miso.iso.copyDOMIntoVTree(true, body, currentNode, document);
  expect(currentNode.children[0].children[0].domRef).toEqual(txt);
...
test('Should fail to mount on a text node', () => {
  ...
  var currentNode = vnodeKids('div', [ vnodeKids('div', [ vtext("foo") ]) ]);
  var succeeded = miso.iso.copyDOMIntoVTree(true, misoTxt, currentNode, document);
  expect(succeeded).toEqual(false);
});
...
test('Should mount on an empty body', () => {
  ...
  var body = document.body;
  var currentNode = vnodeKids('div', [ vnodeKids('div', [ vtext("foo") ]) ]);
  var succeeded = miso.iso.copyDOMIntoVTree(true, null, currentNode, document);
  expect(succeeded).toEqual(false);
});
