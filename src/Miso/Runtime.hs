  ...
  componentVTree <- do
    ...
    case hydrate of
      Draw -> do
        ...
        liftIO $ when isRoot (atomicWriteIORef globalVTree vtree)
      Hydrate -> do
...
removeFromDelegatedEvents componentId eventsToRemove logLevel = do
  ...
  liftIO $ modifyMVar_ delegatedEvents $ \currentEvents -> do
    ...
    requiredEvents <-
      M.unions . fmap componentEvents . IM.elems . IM.delete componentId <$>
...
data ComponentState model action
  ...
  , componentEvents :: Events
  -- ^ t'Events' that the t'Miso.Types.Component' will listen on
  }
-----------------------------------------------------------------------------
...
globalVTree = unsafePerformIO (newIORef undefined)
-----------------------------------------------------------------------------
