...
play
  :: Media
  -- ^ Media element to start playing
  -> IO ()
...
--
-- Sets the media stream source on an audio or video element.
srcObject
  :: Stream
  -- ^ Media stream to attach
  -> Media
  -- ^ Media element to update
  -> IO ()
