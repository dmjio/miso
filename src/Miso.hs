...
-- @
--
-- 'VNode' and 'VText' have a one-to-one mapping from the virtual DOM to the physical DOM. The 'VComp' and 'VFrag' constructors are abstract (live only on the virtual DOM) and do not contain a reference to the physical DOM. 'VPort' likewise lives only on the virtual DOM, but carries an @'IO' 'DOMRef'@ that resolves to the out-of-tree /location/ into which its child is rendered. The existential 'SomeComponent' is what allows embedding polymorphic 'Component' within a 'View'.
--
-- @
...
-- * 'portal', 'vport', 'portal_', 'vport_' — build a 'VPort'
