...
function replace<T>(c: VTree<T>, n: VTree<T>, parent: T, context : DrawingContext<T>): void {
  ...
  if (n.type === 'vtext') {
    ...
    createElement(n, parent, context, (newChild: T) => {
...
// ** </> recursive calls to hooks
export function callCreated<T>(obj: VTree<T>, parent: T, context: DrawingContext<T>): void {
  if (obj['onCreated']) obj['onCreated'](obj.domRef);
  if (obj.type === 'vcomp') mountComponent(obj, parent, context);
}
...
export function populate<T>(c: VTree<T>, n: VTree<T>, parent: T, context: DrawingContext<T>): void {
  if (n.type !== 'vtext') {
    if (!c) c = vnode({});
...
function createElement<T>(obj: VTree<T>, parent: T, context: DrawingContext<T>, attach: (e: T) => void): void {
...
function mountComponent<T>(obj: VComp<T>, parent: T, context: DrawingContext<T>): void {
  if (obj.onBeforeMounted) obj.onBeforeMounted();
  // Call 'onBeforeMounted' before calling 'mount'
  obj.mount(parent, (componentId: ComponentId, componentTree: VTree<T>) => {
    // mount() gives us the VTree from the Haskell side, so we just attach it here
    // to tie the knot (attach to both vdom and real dom).
    // when we mount, depending on what's returned, we might inline it into
    // the existing child node list (e.g. if a 'vfrag' is returned)
    switch (componentTree.type) {
      ...
      case "vtext":
        ...
        obj.children.push(componentTree);
        break;
      case "vcomp": // dmj: recursive case, keep drilling until we get to 'vnode' / 'vtext'
        obj.children.push(componentTree);
...
function create<T>(obj: VTree<T>, parent: T, context: DrawingContext<T>): void {
  if (obj.type === 'vtext') {
    ...
    createElement<T>(obj, parent, context, (child: T) => {
      ...
      */
      if (found) {
        ...
        -> [ b e a j   ] <- new children
             ^
        */ else {
        createElement<T>(nFirst, parent, context, (e: T) => {
