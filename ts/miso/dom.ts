...
function diffNodes(c: VTree, n: VTree, parent: Element, context: Context): void {
  ...
  if (c['type'] === 'vtext') {
    ...
    // dmj: By default all component-id are generated by the miso runtime itself
    // The 'component-id' is *not* a stable name. Which means we create new component-id on each diff
    // This means we cannot tell the difference between swapping component 'a', for component 'b' vs. keeping component 'a' and just giving it a new component-id.
  ...
  var componentIdCheck = function (n,c) {
      if (n['type'] === 'vcomp' && !n['component-id'].startsWith('miso-component-id')) {
          return n['component-id'] === c['component-id']
      }
      return true;
  };
...
function mountComponent(obj: VTree, context: Context): void {
  ...
  obj['mount'](obj['domRef'], (component: VNode) => {
