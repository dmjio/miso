...
function diffNodes(c: VTree, n: VTree, parent: Element, context: Context): void {
  ...
  if (c['type'] === 'vtext') {
    ...
    // dmj: if the component is not autogenerated (this means you can send a message to it) we diff the 'data-component-id' since it's a stable name
    // If the component *is* autogenerated (i.e. nameless), the 'data-component-id' is /not/ a stable name and user's must rely on using "key" if they want to replace unnamend components in a reliable way. When in doubt, use 'component' with a unique name, or when using 'component_' use embedKeyed_ with a unique key.
  var componentIdCheck = function (n,c) {
      if (n['type'] === 'vcomp' && !n['data-component-id'].startsWith('miso-component-id')) {
          return n['data-component-id'] === c['data-component-id']
      }
      return true;
  };
...
function mountComponent(obj: VTree, context: Context): void {
  ...
  context['setAttribute'] (obj['domRef'] as Element, 'data-component-id', componentId);
