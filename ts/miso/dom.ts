...
function diffNodes(c: VTree, n: VTree, parent: Element): void {
  ...
  if (c['type'] === 'vtext') {
    ...
    // If the component *is* autogenerated (i.e. nameless), the 'data-component-id' is /not/ a stable name and user's must rely on using "key" if they want to replace unnamend components in a reliable way. When in doubt, use 'component' with a unique name, or key with a unique key.
  var componentIdCheck = function (n,c) {
      if (n['type'] === 'vcomp' && typeof n['data-component-id'] === 'string' && !n['data-component-id'].startsWith('miso-component-id')) {
