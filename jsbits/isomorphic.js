...
window['miso']['iso'] = (function () {
  ...
  var copyDOMIntoVTree = function(logLevel,mountPoint, vtree, doc) {
    ...
    if (!walk(logLevel,vtree, node, doc)) {
      ...
      vtree['domRef'] = node;
      window['miso']['vdom']['populate'](null, vtree, doc);
      return false;
    }
    if (logLevel) {
  ...
  var walk = function(logLevel, vtree, node, doc) {
    ...
    window['miso']['vdom']['callCreated'](vtree);
    ...
    for (var i = 0; i < vtree.children.length; i++) {
      ...
      if (vdomChild.type === 'vtext') {
        ...
        vdomChild['domRef'] = domChild;
        if(!walk(logLevel, vdomChild, domChild, doc)) return false;
      }
    }
    return true;
  };
